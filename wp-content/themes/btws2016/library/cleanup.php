...
if ( ! function_exists( 'btws_start_cleanup' ) ) :
function btws_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'btws_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'btws_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'btws_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'btws_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'btws_gallery_style', 'btws_gallery_style' );

}
add_action( 'after_setup_theme','btws_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'btws_cleanup_head' ) ) :
function btws_cleanup_head() {
...
// Remove WP version from RSS.
if ( ! function_exists( 'btws_remove_rss_version' ) ) :
function btws_remove_rss_version() { return ''; }
endif;

// Remove injected CSS for recent comments widget.
if ( ! function_exists( 'btws_remove_wp_widget_recent_comments_style' ) ) :
function btws_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'btws_remove_recent_comments_style' ) ) :
function btws_remove_recent_comments_style() {
...
if ( ! function_exists( 'btws_gallery_style' ) ) :
function btws_gallery_style( $css ) {
...
if ( ! class_exists( 'btws_img_rebuilder' ) ) :
	class btws_img_rebuilder {
	...
	$btws_img_rebuilder = new btws_img_rebuilder;
